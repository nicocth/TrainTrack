...
class ExerciseCardTraining extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    final List<String> headers = ["\u2713", S.current.series, S.current.kg_text, S.current.reps_text];
            ...
            Column(
              ...
              children: [
                Row(
                  ...
                  ).expand((widget) => [widget, SizedBox(width: 25)]).toList()
                    ..removeLast(),
                ),
                const SizedBox(height: 5),
                Column(
                  children: List.generate(customExercise.sets.length, (index) {
                    final isChecked = trainingSessionState.completedSets[customExercise.order].contains(index);
                    return Container(
                      color: isChecked ? Color.fromRGBO(115, 255, 0, 0.2) : null,
                      child: Row(
                        children: [
                          SizedBox(
                            width: 30,
                            child: Checkbox(
                              ...
                              onChanged: (value) {
                                if (value == true) {
                                  trainingSessionNotifier.markSetAsCompleted(customExercise.order, index);
                                }else{
                                  trainingSessionNotifier.markSetAsNotCompleted(customExercise.order, index);
                                }
                              },
                            ),
                          ),
                          const SizedBox(width: 25),
                          SizedBox(width: 50, child: Center(child: Text("${index + 1}"))),
                          const SizedBox(width: 25),
                          SizedBox(
                            width: 50,
                            child: TextField(
